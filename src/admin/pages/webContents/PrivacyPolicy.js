...
const PrivacyPolicy = () => {
  ...
  const submitHandler = (evt) => {
    ...
    let url = `${Config.SERVER_URL}/privacyPolicy`;
    if (privacyPolicy) {
      url += `/${formData._id}`;
    }
    let method = privacyPolicy ? "PUT" : "POST";
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await fetch(`${Config.SERVER_URL}/privacyPolicy`);
