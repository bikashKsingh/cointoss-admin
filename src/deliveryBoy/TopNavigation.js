...
function TopNavigation() {
  ...
  useEffect(() => {
    document.title = "Delivery Boy Pannel";
  }, []);
    ...
    const getDate = async () => {
      try {
        const result = await fetch(
          Config.SERVER_URL + "/deliveryBoys/profile",
          {
            method: "GET",
            headers: {
        ...
        if (data.status == 200) {
          ...
          localStorage.setItem(
            "deliveryBoy",
            JSON.stringify({ name: result.body.name })
          );
        } else {
      ...
      {state && (
        <nav className="navbar top-navbar navbar-expand-md navbar-light">
          ...
          <div className="navbar-header">
            <Link className="navbar-brand" to="/supervisor">
              {/* <!-- Logo icon --> */}
              <b>
                {/* <!-- Dark Logo icon --> */}
                <img
