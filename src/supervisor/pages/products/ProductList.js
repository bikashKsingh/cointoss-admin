...
const ProductList = (props) => {
  ...
  const [isAllProductsLoding, setIsAllProductsLodig] = useState(true);
  ...
  useEffect(() => {
    let url = `${Config.SERVER_URL}/products?page=${pagination.page}&limit=${pagination.limit}&status=${status}`;
          ...
          return (
            <>
              <Link
                ...
                to={{
                  pathname: `/supervisor/product/details/${value}`,
                }}
              >
  ...
  const data = React.useMemo(() => {
    return allProducts.map((data, index) => {
      return {
        sn: ++index,
        name: data.name,
