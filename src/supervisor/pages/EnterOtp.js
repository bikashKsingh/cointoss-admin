...
function EnterOtp() {
  ...
  const resendOtpHandler = async (evt) => {
    ...
    try {
      const response = await fetch(
        Config.SERVER_URL + "/supervisors/findAccount",
        {
          method: "POST",
  ...
  const verifyOtpHandler = async (evt) => {
    ...
    try {
      ...
      const response = await fetch(
        Config.SERVER_URL + "/supervisors/verifyOTP",
        {
          method: "POST",
      ...
      if (result.status == 200) {
        ...
        history.push("/supervisor/create-password");
      } else {
            ...
            <div className={"card-body"}>
              ...
              <form onSubmit={verifyOtpHandler} className={"form-material"}>
                <div className={"form-group"}>
                  ...
                  <div className={"mt-3"}>
                    <Link to={"/supervisor/login"}>Back to Login?</Link>
                  </div>
                </div>
              </form>
            </div>
